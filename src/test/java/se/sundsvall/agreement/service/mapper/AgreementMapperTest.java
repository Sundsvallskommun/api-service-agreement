...
class AgreementMapperTest {
	...
	void testToAgreementPartiesAgreementsForAgreementsWithNoEndDate() {
		...
		assertThat(agreementParty).isNotNull().extracting(AgreementParty::getAgreements).asList().hasSize(1);
	...
	/**
	 * Utility method returning dataWarehouseReader response with list of size x agreements belonging to same customer where binding is false and mainAgreement is true
	 */
